...
type CartPageProps = {
  cart: Product[]; // Ensure consistency with the Product type
};

export default function CartPage({ cart }: CartPageProps) {
  const calculateTotal = () => {
    return cart.reduce((acc, item) => acc + item.price * (item.quantity || 1), 0); // Default to 1 if quantity is undefined
  };
